...
def run():
    ...
    for sensors_in_action in list_combinations[-2:-1]:
        for model in ['resnet', 'deepconv', 'convlstm']:
            for val in ['kfold', 'lro', 'lwo']:
                train(model_name=model, validation=val, sensors=list(sensors_in_action), dataset='22labels')
    ...
    #         # if idx == 0 and jdx < 2:
    #         #     continue
    #         train(model_name=model, validation=val, sensors=['RF'], dataset='22labels') # sensors=['LF', 'LW', 'ST', 'RW', 'RF'], dataset='22labels')
...
def fit_kfold_iteration(model_config: JensModelBuilder, train_windows: list[Window],
    ...
    # log(f"Classification Report: {evaluation['classification_report']}")
